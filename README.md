...
**Открыть приложение: [http://localhost:8000/web/index.html](http://localhost:8000/web/index.html).**
...
После запуска открыть [http://localhost:8000/web/index.html](http://localhost:8000/web/index.html). Сервер работает до `Ctrl+C`. Команда `python -m pipeline --data data --out out`, выполненная интерпретатором окружения, воспроизводит все аналитические артефакты без ручного редактирования данных.
...
При прямом запуске Python переменные задаются в окружении процесса: файл `.env` автоматически не читается. Пример для PowerShell перед запуском сервера:
...
Проект использует датасет и стартовый код организаторов из `data/` и `starter/`. Аналитические модули решения находятся в `pipeline/`, сервер и инструменты ассистента — в `api/`, интерфейс — в `web/`. При подготовке документации использован AI-агент Codex для анализа реализации, проверки запуска и оформления README.
