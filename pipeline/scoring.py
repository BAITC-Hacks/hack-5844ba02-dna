...
def score_nodes(features: pd.DataFrame, cfg: dict) -> pd.DataFrame:
    ...
    weighted = {name: components[name] * weights[name] for name in weights}
    multiplier = result.is_seed.map(
        lambda is_seed: cfg["scoring"]["seed_multiplier"] if is_seed else 1.0
    )
    raw = sum(weighted.values()) * multiplier
    normalizer = float(raw.max())
    if normalizer:
        contributions = {name: values * multiplier / normalizer for name, values in weighted.items()}
        result["priority_score"] = sum(contributions.values())
    else:
        contributions = {name: values * 0.0 for name, values in weighted.items()}
        result["priority_score"] = 0.0
    # Store final, not pre-normalisation, contributions.  Their sum reconstructs
    # priority_score, including the seed multiplier.
    result["score_breakdown"] = [json.dumps({key: float(values.iloc[i]) for key, values in contributions.items()}, ensure_ascii=False, sort_keys=True) for i in range(len(result))]
    return result
...
def top_nodes(features: pd.DataFrame, cfg: dict) -> pd.DataFrame:
    ...
    for rank, row in enumerate(ordered.itertuples(index=False), 1):
        values = {"seed_flow_kzt": f"{row.seed_flow_kzt / 1_000_000:.1f} млн", "n_seed_upstream": row.n_seed_upstream, "role": row.role, "betweenness": row.betweenness, "degree": row.in_deg + row.out_deg, "temporal": f"сопоставимые быстрые переводы {row.fast_pass_share:.0%}"}
